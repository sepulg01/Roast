...
- Validacion productiva actual: `npm run smoke:worker-production -- --base-url https://caferoast.cl` falla en `GET /api/health expected HTTP 200, received HTTP 404`, confirmando que produccion aun tiene el Worker anterior hasta que GitHub Actions despliegue con secretos productivos.
- Completado totalmente: automatizacion de deploy persistente, sync de secretos, healthcheck, smoke test, migracion mockeada a Resend, fallback legacy Apps Script, documentacion y cobertura automatizada local.
- Parcial: no se pudo confirmar email real a `gosepulvedah@gmail.com` ni pedido real `NO PREPARAR` porque produccion todavia responde con el Worker anterior y los secretos productivos deben cargarse en GitHub Environment `production` antes del deploy/smoke.
- Pendiente/deferido: cargar en GitHub Environment `production` los secretos requeridos (`CLOUDFLARE_*`, `GOOGLE_*`, `PUBLIC_BASE_URL`, `RESEND_*`), ejecutar `Sync Worker Secrets`, dejar correr `Deploy Worker`, repetir el smoke productivo y entonces hacer la prueba real con `gosepulvedah@gmail.com`, `Avenida Consistorial 2320, Casa 1, Peñalolén`, nota `NO PREPARAR`.
- No se ejecuto refresh de stacks por instruccion explicita de este plan.
