...
## 2026-04-30 - Cierre temporal por WhatsApp y email

- Se reemplazo el cierre activo del checkout por `Email + WhatsApp`: el cliente sigue armando el pedido, agregando items, dejando sus datos y revisando el total; el paso final ahora llama `POST /api/order-contact-requests`, exige que Apps Script reciba el email operativo, registra `contact_requested`, emite `order_contact_requested` y redirige al WhatsApp de soporte con contexto del pedido.
- `apps-script/Code.js` ahora arma un email operativo mas completo para `contacto@caferoast.cl`, incluyendo cliente, email, telefono, comuna, direccion, referencia, notas, items, subtotal, despacho, total, origen, canal y link de WhatsApp.
- Se actualizaron precios fallback y visibles de ambos productos a `250g=$9.900`, `500g=$18.900`, `1kg=$32.900`; el combo fallback del quiz queda en `$19.800 CLP`. Los mocks funcionales tambien reflejan esos valores.
- Se agrego la compuerta de aprobacion de textos visibles: `scripts/check-visible-copy-approval.mjs`, `.githooks/pre-commit`, `copy-approvals/TEMPLATE.md` y `copy-approvals/2026-04-30-checkout-whatsapp-email.md`. Tambien se activo `core.hooksPath=.githooks` en el repo local y el check cubre tanto cambios staged como working tree cuando se corre fuera del hook.
- Se ajustaron copys visibles y documentacion operativa para no prometer Flow como cierre activo, manteniendo los endpoints Flow documentados como codigo disponible para reactivacion posterior.
- Se endurecio la maquina de estados: el cierre por contacto solo acepta pedidos `draft` o `manual_review`, es idempotente para `contact_requested`, y `POST /api/payment-links` ya no puede crear links para pedidos que salieron de `draft` salvo reutilizar links previos `link_sent` o `pending_payment`.
- Validacion realizada: `npm run test:static`, `npm --prefix worker run check`, `npm run test:functional -- --reporter=line` con 96 pruebas pasando, `npm run check:copy-approval` fallando correctamente con `index.html` staged sin aprobacion, `npm run check:copy-approval` pasando con la aprobacion staged, busquedas `rg` para copys activos de Flow y precios antiguos, y `git diff --check`.
...
- Se completo el despliegue productivo posterior con Wrangler `4.86.0`, quedando publicada la version `6d653f4f-4d45-4a01-9ba6-caa119ba8634` del Worker `roast` en `caferoast.cl/api/*` y `caferoast.cl/pago/retorno`.
- Se cargaron/configuraron los secretos productivos necesarios para Google Sheets y Google Geocoding; luego `GET /api/public-catalog` respondio `200` con catalogo, comunas y precios `11900/19900/34900`.
...
- Se endurecio la UI de confirmacion para preferir `confirmation_number` / `order_number` y no mostrar IDs crudos `roast_*` cuando el backend productivo aun este desfasado.
- Se actualizaron `README.md`, `scripts/sync-sheet-readme.mjs`, `worker/.dev.vars.example`, `worker/package-lock.json` y la aprobacion `copy-approvals/2026-05-04-persistent-worker-resend.md`.
- Validacion realizada: ciclo rojo/verde Worker para `/api/health`; ciclo rojo/verde Worker para Resend, idempotencia y fallback sin `roast_*` crudo; `npm run test:worker` con 18 pruebas pasando; `npm run test:functional -- tests/functional/checkout.spec.js --project=chromium --reporter=line` con 22 pruebas pasando; `npm run test:static`; `npm run test:functional -- --reporter=line` con 138 pruebas pasando; `npm run check:copy-approval`; `git diff --check`; `cd worker && npm exec -- wrangler --version`.
- Validacion productiva previa al push: `npm run smoke:worker-production -- --base-url https://caferoast.cl` fallaba en `GET /api/health expected HTTP 200, received HTTP 404`, confirmando que produccion aun tenia el Worker anterior.
- Validacion productiva posterior al push: `npm run smoke:worker-production -- --base-url https://caferoast.cl` paso completo; luego se creo el pedido real de prueba `roast_20260504_114349_ypoz2` para `gosepulvedah@gmail.com`, direccion `Avenida Consistorial 2320, Casa 1, Peñalolén`, nota `NO PREPARAR - PRUEBA TECNICA CODEX 2026-05-04`, estado `pending_transfer`, total `$15.400`, numero visible `0405481`.
- `GET /api/orders/roast_20260504_114349_ypoz2` respondio `200` con `order_number` y `confirmation_number` igual a `0405481`, confirmando que el endpoint publico ya no expone el numero crudo como confirmacion.
- Validacion productiva posterior a cargar secretos Resend en Cloudflare: `GET /api/health` respondio con `configuration.resend=true` y `configuration.notifications=true`, y `npm run smoke:worker-production -- --base-url https://caferoast.cl` paso completo.
- Se creo un segundo pedido real de prueba para validar el flujo ya con Resend configurado: `roast_20260504_123912_1wwx6`, email `gosepulvedah@gmail.com`, direccion `Avenida Consistorial 2320, Casa 1, Peñalolén`, nota `NO PREPARAR - PRUEBA TECNICA CODEX 2026-05-04 - VALIDACION RESEND`, total `$15.400`, numero visible `0405064`.
- `GET /api/orders/roast_20260504_123912_1wwx6` respondio `200` con `order_number` y `confirmation_number` igual a `0405064`, confirmando que el endpoint publico mantiene el numero visible de solo digitos tras el despliegue con Resend.
- Completado totalmente: automatizacion de deploy persistente, sync de secretos, healthcheck, smoke test, migracion mockeada a Resend, fallback legacy Apps Script, documentacion, cobertura automatizada local, deploy productivo y prueba real de creacion/consulta de pedido.
