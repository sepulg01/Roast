...
function buildReadmeRows() {
  return [
    ...
    ['Arquitectura', 'Telegram Bot API', 'Alertas y botones operativos internos', 'Envia alertas para pending_transfer, paid y delivering; los botones llegan por webhook firmado'],
    ...
    ['Ventas', 'order_number', 'Numero visible de pedido', 'Formato unico DDMMRRR como texto; order_id sigue siendo la llave interna'],
    ['Eventos', 'notification_results_json', 'Resultado de canales de notificacion', 'JSON con email/Telegram; agregar header en la hoja si falta'],
    ...
    ['Worker endpoints', 'POST /api/admin/orders/:order_id/status', 'Acciones operativas firmadas', 'Permite pending_transfer -> paid|expired, paid -> delivering y delivering -> delivered'],
    ...
    ['Worker endpoints', 'POST /api/telegram/webhook', 'Webhook Telegram', 'Verifica secret header y procesa callbacks firmados desde operadores autorizados'],
    ...
    ['Worker endpoints', 'GET /api/health', 'Healthcheck productivo', 'Expone flags y booleans de configuracion para Google, Resend, admin actions y Telegram sin filtrar secretos'],
    ['', '', '', ''],
    ...
    ['Variables y secretos', 'TELEGRAM_BOT_TOKEN', 'Worker secret opcional', 'Token del bot creado en BotFather para alertas operativas internas'],
    ['Variables y secretos', 'TELEGRAM_CHAT_ID', 'Worker secret opcional', 'Chat o grupo operativo donde el bot envia alertas'],
    ['Variables y secretos', 'TELEGRAM_WEBHOOK_SECRET', 'Worker secret opcional', 'Secret token validado desde X-Telegram-Bot-Api-Secret-Token'],
    ['Variables y secretos', 'TELEGRAM_ACTION_SECRET', 'Worker secret opcional', 'Firma payloads compactos de botones operativos Telegram'],
    ['Variables y secretos', 'TELEGRAM_OPERATOR_IDS', 'Worker secret opcional', 'IDs numericos autorizados para ejecutar botones Telegram'],
    ...
    ['Estados operativos', 'delivered', 'Pedido entregado', 'Se marca desde el panel operativo y usa Ventas.delivered_at'],
    ...
    ['Checklist de prueba', '4', 'Validar acciones operativas', 'Panel operativo debe cambiar pending_transfer a paid|expired, paid a delivering y delivering a delivered sin duplicar eventos criticos'],
