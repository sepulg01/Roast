...
function buildReadmeRows() {
  return [
    ...
    ['Arquitectura', 'Cierre temporal', 'Email + WhatsApp', 'El Worker registra contacto y Apps Script envia el email operativo'],
    ['Arquitectura', 'Flow', 'Proveedor de pago pausado', 'El codigo para links y sincronizacion queda disponible para reactivacion posterior'],
    ...
    ['Worker endpoints', 'POST /api/order-contact-requests', 'Cierre temporal por email + WhatsApp', 'Exige email operativo antes de actualizar Ventas a contact_requested'],
    ['Worker endpoints', 'POST /api/payment-links', 'Codigo legado Flow', 'Crea links solo para draft; reutiliza link_sent o pending_payment'],
    ...
    ['Variables y secretos', 'APPS_SCRIPT_WEBHOOK_URL', 'Worker secret requerido para cierre temporal', 'Activa email operativo junto al shared secret'],
    ['Variables y secretos', 'APPS_SCRIPT_SHARED_SECRET', 'Worker secret y Script Property requerido', 'Debe coincidir entre Worker y Apps Script'],
    ['', '', '', ''],
    ['Estados operativos', 'draft', 'Pedido creado y listo para cierre por contacto', 'Estado inicial cuando la comuna esta cubierta'],
    ['Estados operativos', 'manual_review', 'Pedido requiere revision humana', 'Usado para comunas fuera de cobertura automatica'],
    ['Estados operativos', 'contact_requested', 'Pedido enviado a contacto', 'Estado temporal cuando se envio email operativo y WhatsApp al cliente'],
    ...
    ['Checklist de prueba', '3', 'Enviar pedido a contacto', 'Debe actualizar Ventas a contact_requested, registrar Eventos y disparar email operativo'],
    ['Checklist de prueba', '4', 'Abrir WhatsApp', 'Debe redirigir a https://wa.me/56991746361 con contexto del pedido'],
